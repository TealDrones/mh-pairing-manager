...
parse_argv(int argc, char *argv[], PairingManager& pairing_manager)
{
    ...
    while ((c = getopt_long(argc, argv, "n:m:p:l:i:a:c:k:f:z:e:h", options, nullptr)) >= 0) {
        switch (c) {
        case 'h':
            help(argv[0]);
            exit(0);
...
main(int argc, char *argv[])
{
    ...
    mux.handle("/pair").post([&](served::response& res, const served::request& req) {
        std::cout << timestamp() << "Got pair request: " << req.body() << std::endl;
