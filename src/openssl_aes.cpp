...
//-----------------------------------------------------------------------------
OpenSSL_AES::~OpenSSL_AES() { deinit(); }

//-----------------------------------------------------------------------------
void OpenSSL_AES::init(std::string password, unsigned long long salt, bool use_compression) {
...
//-----------------------------------------------------------------------------
std::string OpenSSL_AES::encrypt(std::string plain_text) {
  ...
  if (_use_compression) {
    int err = compress2(compressed.get(), &dest_len, reinterpret_cast<const unsigned char *>(plain_text.c_str()),
                        source_len, 9);
    if (err != Z_OK) {
      return {};
    }
...
//-----------------------------------------------------------------------------
std::string OpenSSL_AES::decrypt(std::string cipher_text) {
  int f_len = 0;
