...
TEST(pairingmanager_test, _parse_buffer)
{
  ...
  pairing_manager._parse_buffer(cmd, state, buffer1, n, config_pwd, encryption_key, network_id, channel, bandwidth, power);
  EXPECT_EQ("admin\n", cmd);
  EXPECT_EQ(ConfigMicrohardState::PASSWORD, state);

  char buffer2[] = "Password:\n";
  n = sizeof(buffer2) / sizeof(char);
  pairing_manager._parse_buffer(cmd, state, buffer2, n, config_pwd, encryption_key, network_id, channel, bandwidth, power);
  EXPECT_EQ(config_pwd + "\n", cmd);
  EXPECT_EQ(ConfigMicrohardState::CRYPTO_KEY, state);

  char buffer3[] = "Entering\n";
  n = sizeof(buffer3) / sizeof(char);
  pairing_manager._parse_buffer(cmd, state, buffer3, n, config_pwd, encryption_key, network_id, channel, bandwidth, power);
  EXPECT_EQ("AT+MWVENCRYPT=1," + encryption_key + "\n", cmd);
  EXPECT_EQ(ConfigMicrohardState::POWER, state);

  char buffer4[] = "OK\n";
  n = sizeof(buffer4) / sizeof(char);
  pairing_manager._parse_buffer(cmd, state, buffer4, n, config_pwd, encryption_key, network_id, channel, bandwidth, power);
...
TEST(pairingmanager_test, _parse_buffer_error)
{
  PairingManager pairing_manager;
  pairing_manager.link_type = "MH";
  pairing_manager.config_password = "auterionfct";
